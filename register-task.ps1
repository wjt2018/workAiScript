<#
  ...
  every day at 17:50 local time. The script itself decides whether today is a
...
$trigger = New-ScheduledTaskTrigger -Daily -At 17:50

# StartWhenAvailable: if the PC was off at 17:50, run as soon as it's back on.
$settings = New-ScheduledTaskSettingsSet -StartWhenAvailable `
  -ExecutionTimeLimit (New-TimeSpan -Minutes 15) -MultipleInstances IgnoreNew

Register-ScheduledTask -TaskName $taskName `
  -Action $action -Trigger $trigger -Settings $settings `
  -Description "Generate my work daily report (git + calendar + slack) at 17:50 daily; skips non-workdays." `
  -Force | Out-Null

Write-Output "Registered scheduled task '$taskName' (daily 17:50; script skips non-workdays)."
